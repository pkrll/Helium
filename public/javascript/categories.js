$(document).ready(function() {
    ...
    $.fn.updateCategory = function (element) {
        ...
        if (oldValue != newValue) {
            ...
            // Make the request
            xhr.open("POST", "/content/categories/update", true);
            xhr.send(formData);
        }
    }
    ...
    $.fn.onButtonClick = function (element) {
        ...
        // Make the request
        xhr.open("POST", "/content/categories/remove", true);
        xhr.send(formData);
    }
     ...
     * @param   eventObject
     */
    $(".input-categories").on("keydown", function (event) {
        if (event.keyCode == 13) {
            var self = $(this);
            var name = self.val();
            if (name.length > 0) {
                ...
                xhr.open("POST", "/content/categories/add", true);
                xhr.send(formData);
            }
        }
    });
